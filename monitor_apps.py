...
def get_app_info(pkg_name):
    ...
    # 🔥 1. LISTA VIP (100% à prova de falhas para apps conhecidos)
    VIP_ICONS = {
        "bin.mt.plus": "https://iili.io/CSj72j4.png",
    ...
    try:
        ...
        # 🔥 2. LÓGICA DE EXTRAÇÃO CIRÚRGICA (Sem achismos)
        if not info["icon_local"]:
            ...
            if icon_match and files_list:
                ...
                # Se for PNG/WEBP/JPG nativo, pegamos ele direto!
                if full_icon_path.endswith(('.png', '.webp', '.jpg')):
                    icon_internal_path = full_icon_path
                else:
                    # Se for XML, buscamos estritamente uma versão PNG com o mesmo nome! Nada de pegar imagens aleatórias.
            ...
            if icon_internal_path:
                ...
                # 🛡️ 100% SEGURO: Ícone genérico se o app esconder a imagem no XML.
