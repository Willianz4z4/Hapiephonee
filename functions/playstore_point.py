...
# Palavras-chave que a Rede Neural vai aprender a "escutar"
VOCABULARIO = [
    "perfil", "conta", "account", "play points", "pontos do play", 
    "perks", "vantagens", "benefícios", "claim", "reivindicar", 
        ...
        try:
            ...
            for node in root.iter('node'):
                ...
                for i, palavra in enumerate(VOCABULARIO):
                    if palavra in txt: screen_vector[i] = 1.0
        except: pass
        ...
        try:
            ...
            for node in root.iter('node'):
                if node.attrib.get('clickable') == 'true':
                    ...
                    if len(text) > 70 or any(j in text+desc for j in junk_texts) or any(j in res_id for j in junk_ids): continue
        ...
        while True:
            ...
            for step in range(12):
                ...
                for key, data in clickables.items():
                    # Junta a tela e o botão num vetor de 30 posições
                ...
                if random.uniform(0, 1) < self.epsilon:
                    ...
                    print(f"🎲 [Explorando] Botão: {chosen_key.split('|')[1]}")
                else:
                    chosen_key = max(action_scores, key=action_scores.get)
                    print(f"🧠 [Neurônios] Botão: {chosen_key.split('|')[1]} (Nota Prevista: {action_scores[chosen_key]:.1f})")
                ...
                if "com.android.vending" not in (new_xml or ""):
                    ...
                    if any(t in txt_clicado for t in ["claim", "resgatar", "reivindicar"]):
                        ...
                        is_terminal = True
                    elif any(t in txt_clicado for t in ["perks", "vantagens"]): reward = 90.0
                    elif any(t in txt_clicado for t in ["play points"]): reward = 60.0
                    elif any(t in txt_clicado for t in ["perfil", "conta"]): reward = 30.0
            ...
            self.save_model()
            if self.epsilon > self.min_epsilon: self.epsilon -= 0.05
            episode += 1
