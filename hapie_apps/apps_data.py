...
def add_ugclone_config(pacote_alvo, configs):
    ...
    # 1. Parar o app ANTES de ler/escrever!
    executar_root(f"am force-stop {ug_pkg}")

    # 2. Lemos o arquivo real salvo no disco
    ...
    # 4. Transforma em dicionário, atualiza com configs do bot, volta pra string e escapa aspas
    ...
    # 5. Prepara a nova tag XML e injeta na memória
    ...
    # 6. Gravação DIRETA no arquivo original via Root usando 'cat EOF'
    comando = f"""
mkdir -p /data/data/{ug_pkg}/shared_prefs/

cat << 'EOF_XML' > "{master_xml}"
{xml_content}
EOF_XML
...
"""
    ...
    if "sucesso" in saida:
        print(f"[+] XML gravado e protegido com sucesso usando cat EOF!")
        return True
    else:
