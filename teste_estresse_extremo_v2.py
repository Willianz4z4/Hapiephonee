...
def install_clone(pacote_ug):
    """Fluxo para instalação padrão de clone."""
...
def update_clone(pacote_ug):
    """Fluxo para atualização de clone existente (Safe update)."""
...
def iniciar_automacao(modo="update"):
    pacote_ug = "com.ugcloner.xfein"
    dpi_original = obter_dpi_atual()
    DPI_BOT = "380"

    print(f"\n🔥 INICIANDO AUTOMAÇÃO V2 (Modo: {modo.upper()}) 🔥")
    print(f"📐 DPI Original do Usuário: {dpi_original}")

    try:
        # Preparação do Sistema (Comum para os dois fluxos)
        ...
        if achar_e_clicar(tela_inicial, 'content-desc', 'Search apps'):
            ...
            if achar_e_clicar(tela_pesquisa, 'text', nome_app, min_y=100):
                ...
                if clicou_laranja:
                    # Direciona para o fluxo correto escolhido
                    if modo == "install":
        ...
        # Restauração do Sistema (Comum para os dois fluxos)
        print("\n" + "="*60)
        print("🛑 RESTAURAÇÃO DO SISTEMA INICIADA...")
        print("="*60)
        reativar_play_protect()
        reativar_teclado()
        print(f"🔄 Retornando para o DPI original: {dpi_original}")
        executar_root(f"wm density {dpi_original}")
        print("✅ Tudo de volta ao padrão do usuário com total segurança!")

if __name__ == "__main__":
    # Você pode mudar de "update" para "install" aqui
    iniciar_automacao(modo="update")
